...
**Multi-model inference is an excellent tool for model selection**. It relies on computing power and information theory to select best fit models from hundeds to thousands of candidate models. And, it is rapidly becoming the standard for model selection in terrestrial ecology. Use it any time you need to select the best-fit model.
